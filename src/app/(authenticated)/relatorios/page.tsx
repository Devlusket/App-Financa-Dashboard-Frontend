    ...
    <section>
      <div className="mb-7 flex flex-col justify-between gap-5 xl:flex-row xl:items-end">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          {visao === "individual" && relatorio?.porPessoa.length ? <div className="space-y-1.5"><label htmlFor="pessoa-relatorio" className="text-xs text-muted-foreground">Pessoa</label><Select value={pessoaId} onValueChange={(value) => { if (value) setPessoaId(value); }}><SelectTrigger id="pessoa-relatorio" className="w-full sm:w-40"><UserRound /><SelectValue /></SelectTrigger><SelectContent>{relatorio.porPessoa.map((pessoa) => <SelectItem key={pessoa.pessoaId} value={pessoa.pessoaId}>{pessoa.nome}</SelectItem>)}</SelectContent></Select></div> : null}
