  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2"><Label htmlFor="lancamento-categoria">Categoria</Label><Controller control={control} name="categoriaId" render={({ field }) => <Select value={field.value} onValueChange={field.onChange}><SelectTrigger id="lancamento-categoria" className="w-full"><SelectValue placeholder="Categoria">{(value) => categorias.find((categoria) => categoria.id === value)?.nome ?? "Categoria"}</SelectValue></SelectTrigger><SelectContent>{categorias.map((categoria) => <SelectItem key={categoria.id} value={categoria.id}>{categoria.nome}{categoria.ehPoupanca ? " · Poupança" : ""}</SelectItem>)}</SelectContent></Select>} />{errors.categoriaId && <p className="text-xs text-destructive">{errors.categoriaId.message}</p>}</div>
        ...
        <div className="space-y-2"><Label htmlFor="lancamento-pagador">Quem pagou</Label><Controller control={control} name="responsavelPagamentoId" render={({ field }) => <Select value={field.value} onValueChange={field.onChange}><SelectTrigger id="lancamento-pagador" className="w-full"><SelectValue placeholder="Pessoa">{(value) => pessoas.find((pessoa) => pessoa.id === value)?.nome ?? "Pessoa"}</SelectValue></SelectTrigger><SelectContent>{pessoas.map((pessoa) => <SelectItem key={pessoa.id} value={pessoa.id}>{pessoa.nome}</SelectItem>)}</SelectContent></Select>} />{errors.responsavelPagamentoId && <p className="text-xs text-destructive">{errors.responsavelPagamentoId.message}</p>}</div>
      </div>
      <div className="space-y-2"><Label htmlFor="lancamento-status">Status</Label><Controller control={control} name="status" render={({ field }) => <Select value={field.value} onValueChange={field.onChange}><SelectTrigger id="lancamento-status" className="w-full"><SelectValue>{(value) => value === "PAGO" ? "Pago" : "Pendente"}</SelectValue></SelectTrigger><SelectContent><SelectItem value="PENDENTE">Pendente</SelectItem><SelectItem value="PAGO">Pago</SelectItem></SelectContent></Select>} /></div>
